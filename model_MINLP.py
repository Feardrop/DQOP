...
def createModel(repn_DQ=3, constr_DQ=2):
    model = pe.AbstractModel(name="MI(N)LP")
    
    model.instance_name = ""
    model.R_J_func_type = pe.Param(default=1, mutable=True)  # 1-lin / 2-cubic
    
    
    
    # Parameters for the Piecewise Linear Function for DataQualities
    model.func_factor_DQ  = pe.Param(default=2, mutable=True)  # 1-lin / 2-quad
    model.PieceCnt = pe.Param(default=100, mutable=True)
    ...
    # =============================================================================
    # def f(x, minimum, maximum, func_type):
    #     def qualitysqrt(x, minimum, maximum):
    #         """
    #         maximum = z_I_ub[g]
    #         minimum = z_I_lb[g]"""
    #         a = ((x - minimum)**(1/2))/((maximum - minimum)**(1/2))
    #         return a
    #
    #     def qualitylin(x, minimum, maximum):
    #         """
    #         maximum = z_I_ub[g]
    #         minimum = z_I_lb[g]"""
    ...
    #     try:
    #         if func_type == "sqrt":
    #             return qualitysqrt(x, minimum, maximum)
    #         if func_type == "lin":
    #             return qualitylin(x, minimum, maximum)
    #     except:
    #         raise KeyError('++ Invalid Function Type: "'+func_type+'" ++')
    # =============================================================================
    ...
    def bpts_DQ_build(model, g):
        model.bpts_DQ[g] = []
        for i in range(int(pe.value(model.PieceCnt)+2)):
                (model.bpts_DQ[g].append((i**(pe.value(model.func_factor_DQ)) 
                / (pe.value(model.PieceCnt)+1)**(pe.value(model.func_factor_DQ))) 
